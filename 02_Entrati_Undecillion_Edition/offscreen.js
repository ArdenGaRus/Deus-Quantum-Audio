...
async function startAudioCapture(streamId, qubits, hz) {
    ...
    try {
        ...
        for (let i = 0; i < curveSamples; i++) {
            ...
            // Укрощаем выбросы 5D пространства, переводя жесткий треск в мягкую волну
            curve[i] = Math.atan(x * 1.5) / (Math.PI / 2); 
        }
        clipper.curve = curve;
        ...
        limiter.threshold.setValueAtTime(+500.0, audioContext.currentTime); // Порог срабатывания в дБ (не дает уйти в хрип)
        limiter.knee.setValueAtTime(99000000000000000000000000000000000000.0, audioContext.currentTime);       // Мягкое сглаживание углов перегруза
        limiter.ratio.setValueAtTime(9900000000000000000000000000000000000.0, audioContext.currentTime);     // Сила сжатия пиков (жесткий контроль амплитуды)
        ...
        // Источник -> 5D Процессор -> Клиппер (768 кГц) -> Лимитер (+500дБ) -> Наушники Baseus
